...
fn generate_user_methods(
	...
	// Note that we use UFCS syntax to make missing_unitvalue_impl.rs
	// pass
	quote! {
		impl<T> #name<T> {
...
pub fn try_expand(
	...
	let arguments =
		parse2::<QuantityArgs>(TokenStream2::from(attribute))?;
	...
	let struct_def =
		generate_struct_def(name, &input.vis, default_type);
	let unit_value_check = generate_unit_value_assertion(
		&krate,
		unit_path,
		default_type,
	);
	let quantity_impl =
		generate_quantity_trait_impl(name, &krate, unit_path);
