...
use syn::{
    parse::Parse, parse::ParseStream, punctuated::Punctuated, spanned::Spanned, Path, Token,
};
...
impl Parse for QuantityArg {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        match ident.to_string().as_str() {
            ...
            "value_type" => Ok(QuantityArg::ValueType(input.parse()?)),
...
impl Parse for QuantityArgs {
    fn parse(input: ParseStream) -> syn::Result<Self> {
            ...
            match arg {
                QuantityArg::Unit(p) => {
                    if unit.is_some() {
                        return Err(syn::Error::new(
                            p.span(),
                            QuantityError::DuplicateArgument("unit".into()),
                        ));
                    }
                    unit = Some(p);
                }
                QuantityArg::ValueType(t) => {
                    if default_type.is_some() {
                        return Err(syn::Error::new(
                            t.span(),
                            QuantityError::DuplicateArgument("value_type".into()),
                        ));
                    }
                    default_type = Some(t);
                }
        ...
        let unit =
            unit.ok_or_else(|| syn::Error::new(Span::call_site(), QuantityError::MissingUnit))?;
        Ok(QuantityArgs {
            unit,
